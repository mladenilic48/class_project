...
public class EmailServiceImpl implements EmailService {
	...
	public void sendTemplateMessage(VoucherEntity voucher) throws Exception {
		...
		String text = "<html><body>" + "<table style='border:2px solid black'>"
				+ "<tr><th>Buyer</th><th>Offer</th><th>Price</th><th>Expires date</th></tr>" + "<tr><td>"
				+ voucher.getVoucherUser().getFirstName() + " " + voucher.getVoucherUser().getLastName() + "</td>"
				+ "<td>" + voucher.getVoucherOffer().getOfferName() + "</td>" + "<td>"
				+ voucher.getVoucherOffer().getActionPrice() + "</td>" + "<td>" + voucher.getExpirationDate().toString()
